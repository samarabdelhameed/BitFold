export const idlFactory = ({ IDL }) => {
  ...
  const LoanOfferStatus = IDL.Variant({
    'Active' : IDL.Null,
    'Accepted' : IDL.Null,
    'Cancelled' : IDL.Null,
    'Expired' : IDL.Null,
  ...
  const LoanOffer = IDL.Record({
    ...
    'max_borrowable' : IDL.Nat64,
    'created_at' : IDL.Nat64,
    'ltv_percent' : IDL.Nat64,
    'user_id' : IDL.Principal,
    'utxo_id' : UtxoId,
  });
  const Result_LoanOffer = IDL.Variant({ 'Ok' : LoanOffer, 'Err' : IDL.Text });
  ...
  return IDL.Service({
    ...
    'lock_collateral' : IDL.Func([UtxoId], [Result_LoanOffer], []),
